...
from states.test_9_states import test_9

cnt = 0
name = 'test9'

@dp.message_handler(text='Тест самообладания')
async def test9_e(message: types.Message):
    ...
    await message.answer(f'В тесте {len(about_tests[name]["questions"])} вопросов \n'
                         f'Приступить к тесту?', reply_markup=test9_enter)

@dp.message_handler(text='Прервать прохождение теста 9', state='*')
async def test9_answers_end(message: types.Message, state: FSMContext):
    global cnt
    cnt = 0
    await message.answer(f'Тест прерван', reply_markup=kb_test)
    await state.finish()

@dp.message_handler(text='Приступить к тесту 9')
...
@dp.message_handler(state=test_9.answer1)
...
@dp.message_handler(state=test_9.answer2)
...
@dp.message_handler(state=test_9.answer3)
...
@dp.message_handler(state=test_9.answer4)
...
@dp.message_handler(state=test_9.answer5)
...
@dp.message_handler(state=test_9.answer6)
...
@dp.message_handler(state=test_9.answer7)
...
@dp.message_handler(state=test_9.answer8)
...
@dp.message_handler(state=test_9.answer9)
...
@dp.message_handler(state=test_9.answer10)
async def state10(message: types.Message, state: FSMContext):
    ...
    id_ = message.from_user.id + 9
    await tests_results.new_result(id=id_,
                                   user_id=message.from_user.id,
                                   test_number=name,
                                   test_name=about_tests[name]["name"],
                                   results=test9_score(list),
                                   status='created')
    await tests_results.update_result(id=id_, results=test9_score(list))

    await message.answer(f'{test9_score(list)}', reply_markup=kb_test)
    cnt = 0
    await state.finish()
