...
from states.test_7_states import test_7

cnt = 0
name = 'test7'


@dp.message_handler(text='Прервать прохождение теста ШДТС', state='*')
async def test7_answers_end(message: types.Message):
    await message.answer(f'Вы уверены?', reply_markup=ReplyKeyboardMarkup(
    keyboard=[
        [
            KeyboardButton(text='Прервать тест ШДТС')
        ],
        [
            KeyboardButton(text='Продолжить тест ШДТС')
        ]
    ],
    resize_keyboard=True
))

@dp.message_handler(text='Прервать тест ШДТС', state='*')
async def test7_answers_end_(message: types.Message, state: FSMContext):
    global cnt
    cnt = 0
    await message.answer(f'Тест прерван', reply_markup=kb_test)
    await state.finish()

@dp.message_handler(text='Продолжить тест ШДТС', state='*')
async def test7_answers_end__(message: types.Message, state: FSMContext):
    global cnt
    cnt-=1
    await message.answer(f'Вопрос №{cnt+1}: {about_tests[name]["questions"][cnt]}', reply_markup=test7_menu)
    cnt+=1

@dp.message_handler(text='Шкалы депрессии, тревоги и стресса')
async def test7_answers(message: types.Message):
    global cnt
...
@dp.message_handler(state=test_7.answer1)
...
@dp.message_handler(state=test_7.answer2)
...
@dp.message_handler(state=test_7.answer3)
...
@dp.message_handler(state=test_7.answer4)
...
@dp.message_handler(state=test_7.answer5)
...
@dp.message_handler(state=test_7.answer6)
...
@dp.message_handler(state=test_7.answer7)
...
@dp.message_handler(state=test_7.answer8)
...
@dp.message_handler(state=test_7.answer9)
...
@dp.message_handler(state=test_7.answer10)
...
@dp.message_handler(state=test_7.answer11)
...
@dp.message_handler(state=test_7.answer12)
...
@dp.message_handler(state=test_7.answer13)
...
@dp.message_handler(state=test_7.answer14)
...
@dp.message_handler(state=test_7.answer15)
...
@dp.message_handler(state=test_7.answer16)
...
@dp.message_handler(state=test_7.answer17)
...
@dp.message_handler(state=test_7.answer18)
...
@dp.message_handler(state=test_7.answer19)
...
@dp.message_handler(state=test_7.answer20)
...
@dp.message_handler(state=test_7.answer21)
async def state21(message: types.Message, state: FSMContext):
    ...
    id_ = message.from_user.id + 7
    await tests_results.new_result(id=id_,
                                   user_id=message.from_user.id,
                                   test_number=name,
                                   test_name=about_tests[name]["name"],
                                   results=test7_score(list),
                                   status='created')
    await tests_results.update_result(id=id_, results=test7_score(list))

    await message.answer(f'{test7_score(list)}', reply_markup=kb_test)
    data = await state.get_state()
    if data != None:
        cnt = 0
        await state.finish()
